 ...
 * @returns {ReactElement} The rendered Function component
 */
const Function: React.FC<FunctionValueProps> = (props: FunctionValueProps) => {
    const {
        // Core
        value,
        options,
    ...
    const functionOptions = useMemo(() => {
        ...
        if (filtered.length === 0) {
            devLog.warn("[Function] No function category options found");
        }
    ...
    const currentOption = useMemo(() => {
        ...
        if (!match && value) {
            devLog.info(`[Function] No exact match for "${value}", using fallback: ${selectedOption.name}`);
        }
    ...
    const property = useMemo(() => {
        ...
        try {
            return createProperty(currentOption.name as CSSPropertyKey, currentOption.syntax);
        } catch (error) {
            devLog.error(`[Function] Failed to create property for ${currentOption.name}:`, error);
            return undefined;
        }
    },
        [currentOption]
    ...
    const handleArgumentsChange = useCallback((newValue: string): void => {
        if (!extractedValue.name) {
            devLog.warn("[Function] No function name available for value change");
            return;
        }
    ...
    if (!currentOption) {
        devLog.error("[Function] No matching function option available.");
        return null;
    }

    if (!property) {
        devLog.error("[Function] Property creation failed - invalid syntax definition.");
        return null;
    }
    ...
    return (
        <DropdownReveal closeOnChange={false} placeholder={`${extractedValue.name}()`} title="Edit Function">
            ...
            <div className={CSS.Function} role="presentation">
