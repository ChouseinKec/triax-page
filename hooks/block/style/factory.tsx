...
interface StyleFactory {
	 ...
	 */
	renderValue: (propertyName: CSSPropertyKey) => ReactElement | null;
...
export const useStyleFactory = (): StyleFactory => {
	...
	const handleValueChange = useCallback(
		(propertyName: CSSPropertyKey, value: string) => {
	...
	const renderValue = useCallback<StyleFactory['renderValue']>(
		(propertyName) => {
			...
			const property = CSSPropertyDefinitions[propertyName];

			if (!property) {
				console.warn(`Property ${propertyName} is not defined in CSSPropertyDefinitions`);
				return null;
			}
