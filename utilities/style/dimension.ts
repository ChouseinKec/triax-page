...
import { CSSUnitKey } from '@/types/block/style/unit';
...
function isValueDimension(input: string): boolean {
	...
	// Ensure unit is a valid CSSUnitKey value
 ...
 */
function getDimensionType(input: string): CSSUnitType | undefined {
	if (!isValueDimension(input)) return undefined;
	const unit = extractUnit(input) as CSSUnitKey;
