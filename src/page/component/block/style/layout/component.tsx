...
// Types
import type { StylesLayoutProps } from "./types";
...
/**
 * StylesLayout Component
 *
 * A tabbed interface that organizes CSS property editing into logical categories for improved user experience.
 * Uses render functions instead of hooks for better performance - hooks are called once in this parent component.
 * Each tab directly renders grouped property controls using shared GroupLayout and Property components.
 *
 * @returns Rendered tabbed style editor with categorized property controls
 *
 * @note Respects BlockDefinition.allowedStyles to filter properties per block type
 */
const StylesLayout: React.FC<StylesLayoutProps> = ({ blockID }) => {
    // Define tabs with renderers
    const tabItems = [
        { label: TAB_ICONS.display, title: "Display & Layout", content: renderDisplayTab(blockID) },
        { label: TAB_ICONS.size, title: "Size & Position", content: renderSizeTab(blockID) },
        { label: TAB_ICONS.font, title: "Font & Text", content: renderFontTab(blockID) },
        { label: TAB_ICONS.background, title: "Background & Mask", content: renderBackgroundTab(blockID) },
        { label: TAB_ICONS.border, title: "Border & Outline", content: renderBorderTab(blockID) },
        { label: TAB_ICONS.effect, title: "Effects", content: renderEffectTab(blockID) },
    ];

    return (
        <div className={CSS.StylesLayout}>
            <TabGroup items={tabItems} />
        </div>
    );
};

export default memo(StylesLayout);