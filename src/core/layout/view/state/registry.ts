// Types
import type { ViewDefinition, RegisteredViews, ViewKey } from '@/core/layout/view/types';
import type { ActionDefinition, RegisteredActions, ActionKey } from '@/core/layout/view/types';
import type { ValidateResult } from '@/shared/types/result';
import type { BenchKey } from '@/core/layout/bench/types';

// Helpers
import { validateViewDefinition } from '@/core/layout/view/helpers/validators';
import { validateActionDefinition } from '@/core/layout/view/helpers/validators/action';
 ...
 */
class ViewEditorRegistry {
	private views: RegisteredViews = {};
	...
	registerView(viewDefinition: ViewDefinition): ValidateResult<ViewDefinition> {
		...
		if (this.views[viewDefinition.key]) return { valid: false, message: `ViewEditor with key "${viewDefinition.key}" already registered` };
	...
	getRegisteredViews(benchKey?: BenchKey): Readonly<Record<string, ViewDefinition>> {
		...
		const filteredViews: RegisteredViews = {};
	 ...
	 */
	getRegisteredActions(viewKey?: ViewKey): Readonly<RegisteredActions> {
...
const viewportRegistry = new ViewEditorRegistry();
