...
import { setSelectedViewKey, useSelectedViewKey } from "@/src/core/layout/viewport/managers";
...
const ViewSelect: React.FC = () => {
    ...
    const selectedViewKey = useSelectedViewKey(selectedBenchKey);

    const allViews = useMemo(() => Object.values(getRegisteredViews('main')), []);

    const viewOptions = useMemo(() => allViews.map(view => ({
        name: view.title,
        value: view.key,
        icon: view.icon
    })), [allViews]);
    ...
    return (
        <DropdownSelect
            ...
            onChange={(value) => setSelectedViewKey(selectedBenchKey, value)}
            title="Select View"
            placeholder={viewIcon}
