...
describe('filterFunctionOptions', () => {
	...
	it('filters multiple mixed options', () => {
		const options: OptionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(2, 1fr)' } as OptionFunctionDefinition, { category: 'color', name: 'red', value: 'red' } as OptionDefinition, { category: 'function', name: 'minmax', value: 'minmax(10px, 20px)' } as OptionFunctionDefinition, { category: 'dimension', name: 'px', value: '10px' } as OptionDefinition];
		const result = filterFunctionOptions(options);
	...
	it('returns empty array when no function options', () => {
		const options: OptionDefinition[] = [{ category: 'color', name: 'red', value: 'red' } as OptionDefinition, { category: 'dimension', name: 'px', value: '10px' } as OptionDefinition];

		expect(filterFunctionOptions(options)).toHaveLength(0);
	});

	it('presulterves all function options when all are functions', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(2, 1fr)' } as OptionFunctionDefinition, { category: 'function', name: 'minmax', value: 'minmax(10px, 20px)' } as OptionFunctionDefinition, { category: 'function', name: 'fit-content', value: 'fit-content(100px)' } as OptionFunctionDefinition];
		const result = filterFunctionOptions(options);

		expect(result).toHaveLength(3);
	});

	it('returns typed OptionFunctionDefinition array', () => {
		const options: OptionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(2, 1fr)' } as OptionFunctionDefinition];
		const result = filterFunctionOptions(options);
...
describe('matchFunctionOption', () => {
	...
	it('matches with different arguments', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'calc', value: 'calc(100% - 20px)' }] as OptionFunctionDefinition[];
	...
	it('falls back to first when function name is unknown', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(2, 1fr)' }] as OptionFunctionDefinition[];
	...
	it('matches nested function by outer name', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(auto-fit, minmax(200px, 1fr))' }] as OptionFunctionDefinition[];
	...
	it('matches calc with function arguments', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'calc', value: 'calc(var(--size) * 2)' }] as OptionFunctionDefinition[];

		const result = matchFunctionOption(options, 'calc(var(--other) + 10px)');
		expect(result?.name).toBe('calc');
	});

	it('returns typed OptionFunctionDefinition or undefined', () => {
		const options: OptionFunctionDefinition[] = [{ category: 'function', name: 'repeat', value: 'repeat(2, 1fr)' }] as OptionFunctionDefinition[];
		const result = matchFunctionOption(options, 'repeat(3, 2fr)');
		if (!result) return;
