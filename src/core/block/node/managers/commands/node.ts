 ...
 */
export function setBlockNodeElementKey(nodeID: NodeID, elementKey: ElementKey): void {
	// Validate, pick, and operate on necessary data
	const results = new ResultPipeline('[BlockCommands → setBlockNodeElementKey]')
		.validate({
 ...
 */
export function setBlockNodeData(nodeID: NodeID, nodeData: NodeData): void {
	// Validate and pick the blockInstance to update
	const results = new ResultPipeline('[BlockCommands → setBlockNodeData]')
		.validate({
			nodeID: validateNodeID(nodeID),
		})
		.pick((data) => ({
