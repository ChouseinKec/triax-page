...
// Managers
import { useSelectedBlockID, canBlockHaveAttributes } from "@/src/core/block/instance/manager";
...
// Factory
import { renderAttributeRow } from "@/src/core/block/attribute/component/layout/factory";
...
const BlockAttributeTabRender: React.FC = () => {
    ...
    const makeSection = (title: string, category: 'global' | 'schema' | 'accesibility' | 'specific') => (
        <GroupLayout
            dividerTitle={title}
            isExpandable={false}
            content={() => (
                <div className={CSS.Layout}>
                    {selectedBlockID && getPropertyGroup(category).map((p) =>
                        renderAttributeRow({
    ...
    const tabItems = selectedBlockID
        ? [
            { label: TAB_ICONS.global, title: 'Global', content: makeSection('Global', 'global') },
            { label: TAB_ICONS.schema, title: 'Schema', content: makeSection('Schema', 'schema') },
            { label: TAB_ICONS.accessibility, title: 'Accessibility', content: makeSection('Accessibility', 'accesibility') },
            { label: TAB_ICONS.specific, title: 'Specific', content: makeSection('Specific', 'specific') },
        ]
        : [];
