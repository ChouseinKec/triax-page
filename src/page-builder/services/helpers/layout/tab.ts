...
// Helpers
import { validatePanelID } from '@/src/page-builder/services/helpers/layout/panel';
 ...
 * @returns ValidationResult containing validity and the validated TabID if valid
 *
 * @example
 * validateTabID('tab-123') → { valid: true, value: 'tab-123' }
 */
export function validateTabID(tabID: unknown): ValidationResult<TabID> {
 ...
 * @returns ValidationResult containing validity and the validated TabTitle if valid
 *
 * @example
 * validateTabTitle('My Tab') → { valid: true, value: 'My Tab' }
 */
export function validateTabTitle(tabTitle: unknown): ValidationResult<TabTitle> {
 ...
 * @returns ValidationResult containing validity and the validated TabComponent if valid
 *
 * @example
 * validateTabComponent(MyComponent) → { valid: true, value: MyComponent }
 */
export function validateTabComponent(tabComponent: unknown): ValidationResult<TabComponent> {
 ...
 * @returns ValidationResult containing validity and the validated TabIcon if valid
 *
 * @example
 * validateTabIcon(<Icon name="star" />) → { valid: true, value: <Icon name="star" /> }
 */
export function validateTabIcon(tabIcon: unknown): ValidationResult<TabIcon> {
 ...
 * @returns ValidationResult containing validity and the validated TabOrder if valid
 *
 * @example
 * validateTabOrder(1) → { valid: true, value: 1 }
 */
export function validateTabOrder(tabOrder: unknown): ValidationResult<TabOrder> {
 ...
 * @returns ValidationResult containing validity and the validated TabDefinition if valid
 *
 * @example
 * validateTabDefinition({ id: 'tab-1', title: 'My Tab', component: MyComponent, icon: <Icon />, order: 1, panelID: 'panel-1' }) → { valid: true, value: {...} }
 */
export function validateTabDefinition(tabDefinition: unknown): ValidationResult<TabDefinition> {
