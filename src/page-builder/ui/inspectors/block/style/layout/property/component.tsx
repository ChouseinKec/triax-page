...
// Types
import type { BlockStylesPropertyProps } from "@/src/page-builder/ui/inspectors/block/types";
 ...
 */
const BlockStylesProperty: React.FC<BlockStylesPropertyProps> = ({ component, label, hidden, disabled, property, styles }) => {
    if (!component || typeof component !== "function") return devRender.error("[BlockStylesProperty] No component to render");
    if (!property || typeof property !== "string") devLog.warn("[BlockStylesProperty] No property key provided");
    ...
    return (
        <div
            className={CSS.BlockStylesProperty}
            style={styles}
        ...
        >
            ...
            {label && (
                <div className={CSS.BlockStylesProperty__Label}>
                    <span ref={labelRef} aria-label="Property Label">
                        {label}
                    </span>

                    <FloatReveal targetRef={labelRef} position="top">
                        <div className={CSS.BlockStylesProperty__FloatTitle} aria-label="Property Name">
                            {propertyName}
                        </div>

                        <div className={CSS.BlockStylesProperty__FloatDescription} aria-label="Property Description">
                            {propertyDescription}
                        </div>

                        {/* Render action buttons if property and block are selected */}
                        {property && selectedBlockID &&
                            <div className={CSS.BlockStylesProperty__FloatActions} id={`${property}-tools`} role="group" aria-label="Property Actions">
            ...
            <div className={CSS.BlockStylesProperty__Value}>
                {component()}
            </div>
        </div>
    );
...
export default memo(BlockStylesProperty);